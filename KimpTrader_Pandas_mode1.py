...
class Upbit:
    ...
    def marketbuy(self, coin_ticker, amount):
       if(coin_ticker=="btc"):
        vol = upbit.buy_market_order("KRW-BTC", amount)
        uuid = vol['uuid']
        # time.sleep(0.15)
    ...
    def marketsell(self, coin_ticker, volume):
        # 업비트 시장가 전량매도
        if(coin_ticker=="btc"):
            sell_upbit = upbit.sell_market_order("KRW-BTC", volume)
...
class KimpT:
    ...
    def BeginTrade(self, coin_ticker, amount, leverage, target):
        ...
        try:
            if mode==0:
                ...
                df.loc[df.Serial == self.index, ('Coin_ticker', 'Vol_buy_u', 'Vol_sell_b', 'Price_buy_u', 'Price_sell_b', 'Wish_ekp2', 'Exe_ekp', 'Exe_ekp2')] = test_tuple
    ...
    def GetoutTrade(self, coin_ticker, volU, volB, target):
        ...
        if mode==0:
            ...
            df.loc[df.Serial == self.index, ('Coin_ticker', 'Vol_sell_u', 'Vol_buy_b','Price_sell_u','Price_buy_b','Wish_xkp2','Exe_xkp','Exe_xkp2')] = test_tuple           
            # c.close  
        # except:
        #     pass
...
class Present_Kimp():
        ...
        def ekp(self, coin_ticker):
            ...
            if(coin_ticker=="btc"):
                ...
                # 바이낸스 선물 매수호가 가져오기
                orderbook_b = client.futures_order_book(symbol='BTCUSDT')
...
async def exit_coin(cointicker):
    if cointicker=="btc":
        # 업비트 잔량 계산후 매도주문
        remain_btc = upbit.get_balance("KRW-BTC")
        sell_upbit = upbit.sell_market_order("KRW-BTC", remain_btc)
        print(sell_upbit)
...
if os.path.isfile('Addcoin_list.csv'):
    ...
    df = pd.DataFrame({'Serial': [0], 'Addcir': [0], 'Status': [0], 'Coin_ticker': ['btc'],
...
async def catcher(message: types.Message):
    ...
    # subscribe_fmt = [{"ticket":"UNIQUE_TICKET"},{"type":"orderbook","codes":["KRW-BTC.1"]}]
    ...
    while True: 
        ...
        try:
            ...
            if mode==0: 
                ...
                # 데이터 추출 - 우선 2가지 조건을 만족하는 데이터 테이블을 생성
                Is_status = df['Status']==1
                Is_Wish_ekp = df['Wish_ekp2'] >= data1
                Exe_data = df[Is_status & Is_Wish_ekp]

                if(Exe_data.empty==False):
                    for idx, row in Exe_data.iterrows():
                        A=KimpT(row['Serial'],leverage)
                        T_data=A.BeginTrade(row['Coin_ticker'], row['Waitamount'], leverage, row['Wish_ekp2'])
                        df.loc[df.Serial == row['Serial'], ('Status', 'Vol_buy_u', 'Vol_sell_b', 'Exe_ekp2')] = (2, T_data[0][1], 
                        T_data[1][1], T_data[2])
                        await message.answer("거래번호:{}, {}코인 {}원 김프진입거래 실행!\n 희망진입김프:{}, 현재김프:{}\n {}\n<거래실행정보>\n업비트매수가:{}, 업비트매수수량:{}\n바이낸스매도가:{}, 바이낸스매도수량:{}\n진입김프:{}"
                        .format(row['Serial'], row['Coin_ticker'],format(row['Waitamount'],','),format(row['Wish_ekp2'],','),format(round(data1,2),','),now,format(T_data[0][0],','),
                        format(T_data[0][1],','),format(T_data[1][0],','),format(T_data[1][1],','),format(T_data[2],',')))
                        #<거래실행정보>\n업비트매수가:{}, 업비트매수수량:{}\n바이낸스매도가:{}, 바이낸스매수수량:{}\n진입김프:{}"

                # 여기서부터는 탈출거래
                Is_status = df['Status']==2
                Is_Wish_xkp = df['Wish_xkp2'] <= data2
                Exe_data = df[Is_status & Is_Wish_xkp]
                ...
                if(Exe_data.empty==False):
                    for idx, row in Exe_data.iterrows():
                        B=KimpT(row['Serial'],leverage)
                        T_data2=B.GetoutTrade(row['Coin_ticker'], row['Vol_buy_u'], row['Vol_sell_b'], row['Wish_xkp2'])
                        if row['Addcir']==1:
                            df.loc[df.Serial == row['Serial'], ('Status')] = 1
                        else:
                            df.loc[df.Serial == row['Serial'], ('Status')] = 0

                        df.loc[df.Serial == row['Serial'], ('Vol_sell_u', 'Vol_buy_b', 'Exe_xkp', 'Exe_xkp2')] = (T_data2[0][1], 
                        T_data2[1][1], T_data2[2, T_data2[3]])
                        ...
                        await message.answer("거래번호:{}, {}코인 김프탈출거래 실행!\n 희망탈출김프:{}, 현재김프:{}\n{}\n<거래실행정보>\n업비트매도가:{}, 업비트매도수량:{}\n바이낸스매수가:{}, 바이낸스매수수량:{}\n탈출김프:{}"
                        .format(row['Serial'],row['Coin_ticker'],format(row['Wish_xkp2'],','),format(round(data2,2),','), now,format(T_data2[0][0],','),format(T_data2[0][1],','),
                        format(T_data2[1][0],','),format(T_data2[1][1],','),format(T_data2[3],',')))
                        await message.answer("업비트 거래총손익:KRW {}\n바이낸스 거래총손익:USD {}\n바이낸스탈출김프기준 거래원화손익:KRW {}\n탈출김프기준 최종손익:KRW {}"
                        .format(upbit_net_profit,binance_net_profit,format(binance_net_profit_coinfx,','),format(final_profit,',')))
...
async def mar(message: types.Message):
    ...
    try:
        ...
        btc_p = binance.fetch_ticker("BTC/USDT")
...
async def addcoin(message: types.Message):
    ...
    global df
    if mode==0:
        #/addcoin btc,1235,0.7,100000,반복
        # 데이터 집어넣기
        ...
        # pass
        # 데이터 집어넣기 /addcoin btc, 1.5, 2.5, 30000, 반복
        split1=message.text.split("/addcoin")
        split2= split1[1].split(',',5)
        coin_ticker=split2[0].strip()
        Addcir=False
        ...
        try:
            df2 = pd.DataFrame({'Serial': [i], 'Addcir': [Addcir], 'Status': [1], 'Coin_ticker': [coin_ticker],
                'Waitamount': [amount], 'Wish_ekp2': [ekp], 'Wish_xkp2': [xkp]})
            df=pd.concat([df, df2], ignore_index=True)
            i=i+1 #거래번호 증가시키는 명령
            await message.answer("거래가 등록되었습니다. 코인종류:{} , 희망진입김프:{}, 희망탈출김프:{}, 금액:{}".format(coin_ticker, ekp, xkp, format(amount,",")))           
  
        except Exception as e :
            await message.answer('addcoin 함수 mode1 에러 {}'.format(e))
        try:
            
            lines = "사용방법 ex) /addcoin btc, 1.2, 2.5, 1000000, 반복\n/addcoin 코인종류, 희망진입김프, 희망탈출김프, 진입할금액,반복여부(생략가능)\n"
            lines2 = "<현재 등록된 거래목록>"
            data=[]
            data=lines+lines2
            for idx, row in df.iterrows():
                ...
                line="\n거래번호 :{}, 코인: {}, 반복여부:{}\n Low(wish):{}, High(wish):{}\n Low(real):{}, High(real):{}\n진입여부:{}, 금액:{}\n".format(row['Serial'],
                row['Coin_ticker'],cir,row['Wish_ekp2'],row['Wish_xkp2'],row['Exe_ekp2'],row['Exe_xkp2'],stat,format(round(row['Waitamount']),","))
                data=data+line
            await message.answer(data)
        except Exception as e:
            await message.answer("addcoin함수에러".format(e))
